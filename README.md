...
### 1. Download & Install Ollama (Optional Local Neural Model)

Aura runs **100% offline out-of-the-box** using its built-in Offline Standby Simulation Engine. For deep local LLM intelligence:
...
3. Open your terminal or PowerShell and pull your preferred local model:
   ...
   ```
4. Verify Ollama is running locally at `http://127.0.0.1:11434`.
...
- Detect your computer's local IPv4 network address
- Bind to `0.0.0.0` for local and Wi-Fi LAN access
- Launch your browser automatically to `http://localhost:8000`

---

## 🌐 LAN & Wi-Fi Access (Cross-Device)

You can access Aura from your phone, tablet, or another computer on the same Wi-Fi network using your PC's LAN IP address:

```text
Local Access:    http://localhost:8000
LAN Access:      http://<YOUR-PC-IP>:8000
Health Check:    http://<YOUR-PC-IP>:8000/api/health
```
...
To allow other devices on your local network to connect to Aura on port `8000`, open **PowerShell as Administrator** and run:

### Recommended (PowerShell - Restrict to Private Local Network):
```powershell
New-NetFirewallRule -DisplayName "Aura AI Companion (Port 8000)" -Direction Inbound -LocalPort 8000 -Protocol TCP -Action Allow -Profile Private
```

### Alternative (Windows CMD as Administrator):
```cmd
netsh advfirewall firewall add rule name="Aura AI Companion" dir=in action=allow protocol=TCP localport=8000 profile=private
```

> **Security Note:** This rule permits incoming TCP traffic on port 8000 only within your Private/Local network profile. It does NOT expose your computer to the public internet or disable Windows Firewall.

---

## 🐙 Connecting from GitHub Pages or External Static Hosts

If you deploy or open the frontend from **GitHub Pages** (`https://<username>.github.io/...`) or an external host:
1. Click the **🌐 NETWORK** button in the top right header.
2. Enter your PC's backend address:
   - If opening on the same computer: `http://localhost:8000` or `http://127.0.0.1:8000`
   - If opening from another device: `http://<YOUR-PC-IP>:8000`
3. Click **Ping** to test connection, then click **Save & Connect**.
4. The status badge in the header will update to **Backend Online**.
...
NOVA AI/
├── backend/
│   ├── main.py              # FastAPI server, /api/health, SSE streaming, Ollama proxy & static routes
│   ├── config.py            # Aura persona, local IP resolution, default configs
...
├── frontend/
│   ├── index.html           # 3-column responsive SPA layout & Network Modal
│   ├── css/
...
│       └── app.js           # Reactive application orchestrator & API URL resolver
├── requirements.txt         # Backend Python dependencies
├── run.py                   # One-click startup launcher with port & LAN detection
