...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // --- Backend API Base URL Resolver ---
  function getBackendBaseUrl() {
    // 1. User manual override stored in localStorage
    const custom = localStorage.getItem('aura_custom_backend_url');
    if (custom && custom.trim() !== '') {
      return custom.trim().replace(/\/+$/, '');
    }

    // 2. Environment / Runtime Configuration (e.g. from js/config.js, Vercel build, or window.__ENV__)
    const envConfigUrl = (
      (typeof window.__ENV__ !== 'undefined' && window.__ENV__?.API_URL) ||
      (typeof window.APP_CONFIG !== 'undefined' && window.APP_CONFIG?.API_URL) ||
      (typeof window.VITE_API_URL !== 'undefined' && window.VITE_API_URL) ||
      ""
    ).trim();

    if (envConfigUrl !== '') {
      return envConfigUrl.replace(/\/+$/, '');
    }

    // 3. If served directly by backend or via reverse proxy (same origin)
    if (typeof window !== 'undefined' && window.location && window.location.protocol.startsWith('http')) {
      const hostname = window.location.hostname;
      const isVercel = hostname.includes('vercel.app');
      const isGithub = hostname.includes('github.io');

      // If hosted on static cloud host without config.js set, try same origin or localhost
      if (!isVercel && !isGithub) {
        return ''; // relative path e.g. /api/...
      }
    }

    // 4. Default fallback when opened via file:// or disconnected static preview
...
Use bullet points, exact commands, and high-density technical summaries.`
  ...
  // Backend HUD Status Elements
  const backendPulseDot = document.getElementById('backend-pulse-dot');
  const backendStatusText = document.getElementById('backend-status-text');
  const backendEndpointBadge = document.getElementById('backend-endpoint-badge');
  const backendStatusContainer = document.getElementById('backend-status-container');
  const backendConfigBtn = document.getElementById('backend-config-btn');
  ...
  const backendModalStatusText = document.getElementById('backend-modal-status-text');
  ...
  // 5. Backend Health Check & Ollama Probers

  async function checkBackendHealth() {
    const healthUrl = getApiUrl('/api/health');
    const startTime = performance.now();
    try {
      const resp = await fetch(healthUrl, { method: 'GET', cache: 'no-cache' });
      if (resp.ok) {
        const data = await resp.json();
        const latency = Math.round(performance.now() - startTime);
        
        if (!isBackendOnline) {
          showToast("Backend Server connected successfully!", "info");
          isBackendOnline = true;
          // Load sessions & settings on initial connection
          loadSettings();
          loadSessions();
        }

        if (backendPulseDot) {
          backendPulseDot.className = "w-2.5 h-2.5 rounded-full bg-neon-emerald animate-pulse shadow-[0_0_8px_#10B981]";
        }
        if (backendStatusText) {
          backendStatusText.textContent = "Backend Online";
          backendStatusText.className = "text-xs font-mono text-neon-emerald font-semibold";
        }
        if (backendEndpointBadge) {
          const badgeText = data.lan_ip && data.lan_ip !== '127.0.0.1' ? `${data.lan_ip} (${latency}ms)` : `Local (${latency}ms)`;
          backendEndpointBadge.textContent = badgeText;
          backendEndpointBadge.className = "text-[10px] font-mono px-1.5 py-0.5 rounded bg-emerald-950/60 text-neon-emerald border border-emerald-800/60";
        }

        // Check Ollama status when backend is verified healthy
        await checkOllamaStatus();
        scheduleNextHealthCheck(8000);
        return true;
      } else {
      ...
      if (backendPulseDot) {
        backendPulseDot.className = "w-2.5 h-2.5 rounded-full bg-rose-500 animate-ping shadow-[0_0_8px_#F43F5E]";
      }
      if (backendStatusText) {
        backendStatusText.textContent = "Backend Offline – attempting to reconnect...";
        backendStatusText.className = "text-xs font-mono text-rose-400 font-semibold";
      }
      if (backendEndpointBadge) {
        backendEndpointBadge.textContent = "Disconnected";
        backendEndpointBadge.className = "text-[10px] font-mono px-1.5 py-0.5 rounded bg-rose-950/60 text-rose-400 border border-rose-800/60";
      }

      if (ollamaPulseDot) ollamaPulseDot.className = "w-2.5 h-2.5 rounded-full bg-slate-600";
      if (ollamaStatusText) {
        ollamaStatusText.textContent = "Awaiting Backend...";
        ollamaStatusText.className = "text-xs font-mono text-slate-500";
      }
      if (ollamaLatencyBadge) {
        ollamaLatencyBadge.textContent = "--";
        ollamaLatencyBadge.className = "text-[10px] font-mono px-1.5 py-0.5 rounded bg-slate-900 text-slate-600 border border-slate-800";
      }

      scheduleNextHealthCheck(3000);
      return false;
    }
  }
  ...
  function scheduleNextHealthCheck(delayMs) {
    ...
    healthCheckTimer = setTimeout(checkBackendHealth, delayMs);
  }

  async function checkOllamaStatus() {
    try {
      const resp = await fetch(getApiUrl('/api/status'), { cache: 'no-cache' });
      const data = await resp.json();

      if (data.online) {
        if (ollamaPulseDot) ollamaPulseDot.className = "w-2.5 h-2.5 rounded-full bg-neon-emerald animate-pulse shadow-[0_0_8px_#10B981]";
        if (ollamaStatusText) {
        ...
        if (data.models && data.models.length > 0 && modelSelect) {
          if (modelCountLabel) modelCountLabel.textContent = `${data.models.length} installed`;
        ...
        if (ollamaPulseDot) ollamaPulseDot.className = "w-2.5 h-2.5 rounded-full bg-amber-500";
        if (ollamaStatusText) {
        ...
        if (modelCountLabel) modelCountLabel.textContent = "Standby mode";
      }
    } catch (err) {
      if (ollamaPulseDot) ollamaPulseDot.className = "w-2.5 h-2.5 rounded-full bg-amber-500";
      if (ollamaStatusText) {
  ...
  async function loadSettings() {
    try {
      ...
      if (settings.temperature !== undefined && temperatureSlider) {
      ...
      if (settings.voice_rate !== undefined && voiceRateSlider) {
        voiceRateSlider.value = settings.voice_rate;
        if (rateValue) rateValue.textContent = `${settings.voice_rate}x`;
        speechEngine.setRate(settings.voice_rate);
      }
      if (settings.voice_pitch !== undefined && voicePitchSlider) {
        voicePitchSlider.value = settings.voice_pitch;
        if (pitchValue) pitchValue.textContent = settings.voice_pitch;
  ...
  async function deleteSession(sessionId) {
    try {
      await fetch(getApiUrl(`/api/sessions/${sessionId}`), { method: 'DELETE' });
      await loadSessions();
  ...
  async function handleSendMessage(customPrompt = null) {
    ...
    try {
          ...
          if (line.startsWith('data: ')) {
            ...
            try {
              ...
              if (data.token) {
                ...
                if (tokenStreamSpeed) tokenStreamSpeed.textContent = `Streaming: ${tokPerSec} tok/s`;
              }
              if (data.done) {
      ...
      if (tokenStreamSpeed) tokenStreamSpeed.textContent = `Offline Latency: ~0ms`;
    }
  }
  ...
  function updateTTSUI(active) {
    if (!quickTtsToggle || !ttsIcon || !ttsQuickLabel) return;
    if (active) {
      ttsIcon.textContent = "🔊";
  ...
  // 9. Backend Network Modal Management
  function openBackendModal() {
    if (!backendModal) return;
    const currentBase = localStorage.getItem('aura_custom_backend_url') || getBackendBaseUrl() || (window.location.origin.includes('http') ? window.location.origin : '');
    backendUrlInput.value = currentBase;
  ...
  if (backendConfigBtn) backendConfigBtn.addEventListener('click', openBackendModal);
  if (backendStatusContainer) backendStatusContainer.addEventListener('click', openBackendModal);
  if (closeBackendModalBtn) closeBackendModalBtn.addEventListener('click', closeBackendModal);

  document.querySelectorAll('.preset-backend-btn').forEach(btn => {
    btn.addEventListener('click', () => {
      const targetUrl = btn.dataset.url;
      if (targetUrl === 'auto') {
        const envUrl = ((typeof window.__ENV__ !== 'undefined' && window.__ENV__?.API_URL) || '').trim();
        backendUrlInput.value = envUrl || (window.location.origin.startsWith('http') ? window.location.origin : '');
      } else {
  ...
  if (testBackendBtn) {
    testBackendBtn.addEventListener('click', async () => {
      ...
      const testUrl = `${val}/api/health`;
      backendTestResult.textContent = `Pinging ${testUrl}...`;
      backendTestResult.className = 'text-[10px] font-mono text-neon-cyan';
      const startT = performance.now();
      try {
        const resp = await fetch(testUrl, { cache: 'no-cache' });
        const latency = Math.round(performance.now() - startT);
        if (resp.ok) {
          const data = await resp.json();
          backendTestResult.textContent = `✅ Connected! Status: ${data.status}, Latency: ${latency}ms, IP: ${data.lan_ip || 'cloud/local'}`;
          backendTestResult.className = 'text-[10px] font-mono text-neon-emerald';
        } else {
          backendTestResult.textContent = `⚠️ Server returned status HTTP ${resp.status}`;
          backendTestResult.className = 'text-[10px] font-mono text-amber-400';
        }
      } catch (e) {
        backendTestResult.textContent = `❌ Failed to connect: ${e.message}. Is backend running?`;
        backendTestResult.className = 'text-[10px] font-mono text-rose-400';
      }
    });
  }
  ...
  if (saveBackendBtn) {
    saveBackendBtn.addEventListener('click', () => {
      const val = (backendUrlInput.value || '').trim().replace(/\/+$/, '');
      if (val) {
        localStorage.setItem('aura_custom_backend_url', val);
        showToast(`Backend URL saved: ${val}`, 'info');
      } else {
        localStorage.removeItem('aura_custom_backend_url');
        showToast('Backend URL reset to automatic', 'info');
      }
      closeBackendModal();
      checkBackendHealth();
    });
  ...
  if (resetBackendBtn) {
    resetBackendBtn.addEventListener('click', () => {
      ...
      const envUrl = ((typeof window.__ENV__ !== 'undefined' && window.__ENV__?.API_URL) || '').trim();
      backendUrlInput.value = envUrl || (window.location.origin.startsWith('http') ? window.location.origin : '');
      backendTestResult.textContent = 'Reset to default auto-detection.';
      backendTestResult.className = 'text-[10px] font-mono text-slate-400';
      showToast('Backend endpoint reset to default', 'info');
      checkBackendHealth();
    });
  }

  // 10. Event Listeners Wire-up
  if (chatForm) {
  ...
  // Voice Mic Buttons
  if (micBtn) micBtn.addEventListener('click', () => speechEngine.toggleListening());
  if (inlineMicBtn) inlineMicBtn.addEventListener('click', () => speechEngine.toggleListening());
  ...
  if (voiceRateSlider) {
    voiceRateSlider.addEventListener('input', (e) => {
      if (rateValue) rateValue.textContent = `${e.target.value}x`;
  ...
  if (clearHistoryBtn) clearHistoryBtn.addEventListener('click', () => clearAllSessions());
  if (clearCurrentChatBtn) {
    clearCurrentChatBtn.addEventListener('click', async () => {
      ...
      try {
        await fetch(getApiUrl(`/api/sessions/${currentSessionId}`), { method: 'DELETE' });
  ...
  async function handlePullModel(modelName) {
    ...
    try {
      const resp = await fetch(getApiUrl('/api/ollama/pull'), {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
  ...
  if (exportSessionBtn) {
    exportSessionBtn.addEventListener('click', async () => {
      try {
        const res = await fetch(getApiUrl(`/api/export/${currentSessionId}?format=markdown`));
